...
一个面向 ROS 2 入门与机械臂运动学实验的学习工作空间，当前包含双 `turtlesim` 启动、三连杆机械臂 URDF/RViz 展示，以及三连杆运动学、约束逆运动学和关节空间轨迹规划实验。
...
| `two_link_arm_kinematics` | `ament_python` | 计算三连杆运动学状态，使用 OSQP 求解带动态关节速度边界、速度阻尼器和软避障 Slack 的约束逆运动学问题，并提供三次、五次、梯形/三角速度及多关节同步轨迹实验 |
...
运动学节点新增了以下关节轨迹计算与采样功能：

- 三次轨迹：根据各关节速度、加速度限制计算最短持续时间，并采样位置、速度和加速度
- 五次轨迹：使用五次时间缩放函数计算最短持续时间，并采样位置、速度和加速度
- 梯形/三角速度轨迹：自动判断轨迹类型，支持静止状态、正反方向运动和分段采样
- 多关节同步：以最慢关节的持续时间为基准，对其他关节的梯形轨迹进行时间缩放，使各关节同时到达目标位置

当前 `experiment_multi_joint_profile()` 在节点初始化时启用，使用 `q_start = [0, 0, 0]`、`q_goal = [1.0, -0.5, 0.8]`、最大速度 `0.5 rad/s` 和最大加速度 `1.0 rad/s²` 输出同步轨迹、采样状态与终点误差。三次、五次和单关节梯形轨迹实验调用仍保持注释状态。

这些轨迹函数目前只进行计算并输出日志，不会发布关节位置或速度控制命令。

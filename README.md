...
一个面向 ROS 2 入门与机械臂运动学实验的学习工作空间，当前包含双 `turtlesim` 启动、二连杆机械臂 URDF/RViz 展示，以及二连杆正运动学、雅可比与奇异性分析节点。
...
| `two_link_arm_description` | `ament_cmake` | 二连杆平面机械臂 URDF，并启动 `robot_state_publisher`、关节滑块 GUI 和 RViz2 |
| `two_link_arm_kinematics` | `ament_python` | 订阅关节状态，计算末端位置、雅可比、末端速度、奇异值、条件数和可操作度；比较 Moore-Penrose 伪逆与自适应阻尼最小二乘法 |

二连杆模型的连杆长度为 `L1 = 0.5 m`、`L2 = 0.4 m`，与 URDF 和运动学节点中的参数一致。
...
保持二连杆显示程序运行，在另一个已加载工作空间环境的终端执行：
...
节点会根据最小奇异值自动调整阻尼：当 `sigma_min >= 0.10` 时不使用阻尼，否则在最大阻尼 `0.05` 范围内平滑增大阻尼。
...
`build/`、`install/` 和 `log/` 是 `colcon` 生成目录，已通过 `.gitignore` 排除，不需要提交到仓库。
